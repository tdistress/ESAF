...
- After a release readiness record reaches `published`, CI still passes `--baseline-ref` on ordinary maintenance pull requests. Release gates shall allow `published`→`published` same-phase baselines (matching the earlier beta gate pattern), keep publication identity and closed-gate truth immutable on that path, and reject regression to a candidate phase. Requiring every post-publication candidate to transition only from `closure_candidate` blocks tracker sync and other allowlist-free maintenance.
- Closure-path metadata allowlists bind only the `closure_candidate` transition. After the annotated release tag exists, companion updates such as tests, validators, trackers, and Working Draft deepen passes are expected on a `published` baseline and shall not reuse the closure allowlist as a permanent write filter.
- Every new tracked `tests/test_*.py` module shall be registered in `tools/test-shards.json` under the correct shard with a lexicographically sorted module list. An unregistered module fails shard-manifest validation even when direct `unittest` discovery appears green.
- Helpers that locate a prior readiness phase by scanning a fixed `HEAD~N` window fail as soon as the branch stacks more than N commits after the transition. Prefer content-based first-parent search of the readiness record, or inject deterministic fixtures; fail closed when no matching ancestor exists.
- Clarifying `shall` or `should` in companion manuals shall cite a parent obligation that actually establishes the outcome. Plan or design text that invents a mandatory outcome from adjacent sections is a normative-force defect; demote the language or amend the parent control or attribute text first.
- When a data-model clarifying `shall` names fields, the entity attribute table and examples shall include those fields, or the `shall` shall be narrowed to the attributes already defined. Do not invent assessment-record identifier prefixes in examples; reuse the ESAF-1500 patterns and nesting rules unchanged.
- Local Python minor versions can fail qualified-review AST-oracle unit tests that pass under the CI interpreter on the same commit. Compare against exact `origin/main` before treating those failures as a branch regression; keep the hot-path equivalence gate authoritative for that concern.
- A GitHub merge API call may return 403 for the cloud-agent token even when checks are green. Prefer `gh pr merge` when available, then verify the pull request `state` is `MERGED` and the squash or merge commit is on `origin/main` before retrying or reporting failure.
