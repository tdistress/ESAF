...
COMPLETED_GROUPS: tuple[str, ...] = ("M", "T1", "S", "T2", "T3")
...
class CyberEssentialsPlusEsafToExternalMappingTests(unittest.TestCase):
    ...
    def test_draft_catalog_entry_and_counts_are_generated(self) -> None:
        ...
        self.assertEqual(len(entry["provisions"]), 97)
        self.assertEqual(entry["lifecycle"]["events"], [])
        self.assertEqual(catalog["counts"]["mapping_sets"], 2)
        self.assertEqual(catalog["counts"]["provisions"], 213)
        self.assertEqual(catalog["counts"]["relationships"], 47)
        self.assertEqual(catalog["counts"]["negative_dispositions"], 167)
