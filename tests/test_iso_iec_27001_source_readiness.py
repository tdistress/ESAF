...
RIGHTS_REVIEW = (
    ...
    / "superpowers"
    / "reviews"
    / "2026-09-11-iso-iec-27001-publication-rights-review.md"
)
SOURCE_ORACLE = (
    ROOT
    / "docs"
    / "superpowers"
    / "specs"
    / "2026-09-11-iso-iec-27001-source-readiness-oracle.json"
)
TRACEABILITY = (
    ROOT
    / "docs"
    / "superpowers"
    / "reviews"
    / "2026-09-11-iso-iec-27001-2022-mapping-go-no-go-traceability.md"
)
ISO_LANDING = ROOT / "crosswalks" / "iso-iec-27001.md"
...
EXPECTED_READINESS_LINKS = {
    ...
    (
        "../docs/superpowers/specs/"
        "2026-09-11-iso-iec-27001-mapping-readiness-matrix.json"
    ),
    (
        "../docs/superpowers/reviews/"
        "2026-09-11-iso-iec-27001-2022-mapping-go-no-go-review.md"
    ),
    (
        "../docs/superpowers/reviews/"
        "2026-09-11-iso-iec-27001-2022-mapping-go-no-go-traceability.md"
...
EXPECTED_PUBLICATION = {
    "document_reference": "ISO/IEC 27001:2022",
    "format": "PDF",
    "full_title": (
    ...
    "language": "English",
    "publication_family": "ISO/IEC 27001",
    "publisher": (
...
class IsoIec27001PublicationRightsReviewTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_records_reviewer_date_sources_and_hold_disposition(self) -> None:
        self.assertRegex(
            self.review,
            r"\*\*Reviewer:\*\* Codex ISO/IEC 27001 Publication Rights Reviewer R1",
        )
        self.assertRegex(self.review, r"\*\*Review date:\*\* 2026-09-11")
    ...
    def test_minimal_bibliographic_metadata_is_separate_from_mapping_fields(self) -> None:
        ...
        self.assertIn(
            "does not permit ISO/IEC 27001:2022 clause or control identifiers, titles, "
...
class IsoIec27001SourceReadinessOracleTests(unittest.TestCase):
    @classmethod
    ...
    def test_separates_artifact_publication_year(self) -> None:
        self.assertEqual(
            self.oracle["dates"],
            {
                "artifact_publication": {"date": "2022", "precision": "year"},
                "edition": {"label": "3rd edition", "year": "2022"},
            },
        )
    ...
    def test_distinguishes_normative_artifact_from_supporting_interfaces(self) -> None:
        self.assertEqual(
            ...
            {
                "normative": {
                    "artifact": "ISO/IEC 27001:2022 English PDF",
...
class IsoIec27001ReadinessPublicationTests(unittest.TestCase):
    def test_traceability_covers_every_issue_182_obligation(self) -> None:
        ...
        self.assertIn("source-readiness-oracle.json", evidence["I182-D1A"])
        self.assertIn("source artifact SHA-256 is `null`", evidence["I182-D1B"])
        self.assertIn("`ISO-IEC-27001-READINESS-B001`", evidence["I182-D1B"])
        self.assertIn("provision count and inventory digest are `null`", evidence["I182-D1C"])
        self.assertIn("`ISO-IEC-27001-READINESS-B003`", evidence["I182-D1C"])
        self.assertIn("publication-rights-review.md", evidence["I182-D2"])
        self.assertIn("mapping-readiness-matrix.json", evidence["I182-D3"])
        self.assertIn("mapping-readiness-matrix.json", evidence["I182-D4"])
        self.assertIn("mapping-go-no-go-review.md", evidence["I182-D5"])
        self.assertIn("ESAF-1600", evidence["I182-GO1"])
        self.assertIn("does not close issue 182", evidence["I182-GO1"])
        self.assertIn(
            "zero substantive ISO/IEC 27001 mapping artifacts",
            evidence["I182-HOLD1"],
        )
    ...
    def test_traceability_records_catalog_invariance_and_zero_mapping_artifacts(self) -> None:
        ...
        self.assertIn("ISO/IEC 27001 mapping artifacts: `0`", text)
        for mapping_set in catalog["mapping_sets"]:
            ...
            self.assertNotIn("27001", publication_id)
            self.assertFalse("iso" in publication_id and "27001" in blob)
        inventory_hits = list(ROOT.glob("**/*iso*27001*inventory*.json"))
        inventory_hits += list(ROOT.glob("**/*27001*inventory*.json"))
        self.assertEqual(inventory_hits, [])
        mapping_hits = [
            path
            for path in list(MAPPINGS_DIR.rglob("*")) + list(REGISTRY_DIR.rglob("*"))
            if path.is_file()
            and ("iso" in path.name.lower() or "27001" in path.name.lower())
        ]
    ...
    def test_crosswalk_readme_lists_iso_hold_package(self) -> None:
        ...
        self.assertIn("[ISO/IEC 27001](iso-iec-27001.md)", text)
    ...
    def test_ci_covers_readiness_inputs_and_checks_renderer(self) -> None:
        ...
        for event in ("pull_request", "push"):
            ...
            self.assertIn("tools/render_iso_iec_27001_mapping_go_no_go.py", paths)

        self.assertIn("validation_gates", workflow["jobs"])
        steps = workflow["jobs"]["validation_gates"]["steps"]
        runs = [step.get("run") for step in steps]
        renderer = "python tools/render_iso_iec_27001_mapping_go_no_go.py --check"
