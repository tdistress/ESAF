...
from tools.v010_draft_release_gates import (
    GATE_IDS,
    PHASE_GATE_STATES,
    PREVIOUS_PHASE,
    RECORD_RELATIVE,
...
def _previous_phase_ref(phase: object) -> str:
    from tools.v010_draft_release_gates import CLOSURE_ALLOWLIST, changed_paths_since
...
class V010DraftReleaseGatesTests(unittest.TestCase):
    def setUp(self) -> None:
        self.record, self.body = load_readiness_document(RECORD_PATH)

    def test_check_passes_on_current_repo_record(self) -> None:
        command = [sys.executable, "tools/v010_draft_release_gates.py", "--check"]
    ...
    def test_evidence_candidate_ignores_baseline_ref(self) -> None:
        ...
        from tools.v010_draft_release_gates import main

        evidence = _evidence_candidate_record(self.record)
        with patch(
            "tools.v010_draft_release_gates.load_readiness_document",
    ...
    def test_allowlist_helper_reports_disallowed_paths(self) -> None:
        from tools.v010_draft_release_gates import CLOSURE_ALLOWLIST

        changed = {"VERSION.md", "tools/v010_draft_release_gates.py", RECORD_RELATIVE}
        disallowed = sorted(changed - set(CLOSURE_ALLOWLIST))
        self.assertEqual(["tools/v010_draft_release_gates.py"], disallowed)

    def test_published_same_phase_baseline_is_allowed(self) -> None:
        from unittest.mock import patch

        from tools.v010_draft_release_gates import validate_baseline_transition

        if self.record.get("phase") != "published":
            self.skipTest("current readiness record is not published")
        baseline = deepcopy(self.record)
        with patch(
            "tools.v010_draft_release_gates._git_show_text",
