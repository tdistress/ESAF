...
from tools.v017_draft_release_gates import (
    GATE_IDS,
    PHASE_GATE_STATES,
    PREVIOUS_PHASE,
    RECORD_RELATIVE,
...
def _previous_phase_ref(phase: object) -> str:
    from tools.v017_draft_release_gates import CLOSURE_ALLOWLIST, changed_paths_since
...
class V017DraftReleaseGatesTests(unittest.TestCase):
    def setUp(self) -> None:
        self.record, self.body = load_readiness_document(RECORD_PATH)

    def test_check_passes_on_current_repo_record(self) -> None:
        command = [sys.executable, "tools/v017_draft_release_gates.py", "--check"]
    ...
    def test_rejects_prerequisite_disposition_value_drift(self) -> None:
        ...
        self.assertTrue(
            any(
                "prerequisite_dispositions.iso_iec_42001 shall equal 'HOLD'"
                in error
                for error in errors_iso
            )
        )
        drifted_sp = deepcopy(self.record)
        drifted_sp["prerequisite_dispositions"]["nist_sp_800_53"] = "GO"
        errors_sp = validate_record(ROOT, drifted_sp)
        self.assertTrue(
            any(
                "prerequisite_dispositions.nist_sp_800_53 shall equal 'HOLD'"
                in error
                for error in errors_sp
            )
        )
        drifted_cis = deepcopy(self.record)
        drifted_cis["prerequisite_dispositions"]["cis_controls"] = "GO"
        errors_cis = validate_record(ROOT, drifted_cis)
        self.assertTrue(
            any(
                "prerequisite_dispositions.cis_controls shall equal 'HOLD'"
                in error
                for error in errors_cis
            )
        )
        drifted_phase6 = deepcopy(self.record)
        drifted_phase6["prerequisite_dispositions"]["phase6_toolkit"] = (
            "working_draft_second_deepen"
        )
        errors_phase6 = validate_record(ROOT, drifted_phase6)
        self.assertTrue(
            any(
                "prerequisite_dispositions.phase6_toolkit shall equal "
                "'working_draft_third_deepen'" in error
                for error in errors_phase6
            )
        )
    ...
    def test_evidence_candidate_ignores_baseline_ref(self) -> None:
        ...
        from tools.v017_draft_release_gates import main

        evidence = _evidence_candidate_record(self.record)
        with patch(
            "tools.v017_draft_release_gates.load_readiness_document",
    ...
    def test_allowlist_helper_reports_disallowed_paths(self) -> None:
        from tools.v017_draft_release_gates import CLOSURE_ALLOWLIST

        changed = {"VERSION.md", "tools/v017_draft_release_gates.py", RECORD_RELATIVE}
        disallowed = sorted(changed - set(CLOSURE_ALLOWLIST))
        self.assertEqual(["tools/v017_draft_release_gates.py"], disallowed)

    def test_published_same_phase_baseline_is_allowed(self) -> None:
        from unittest.mock import patch

        from tools.v017_draft_release_gates import validate_baseline_transition

        if self.record.get("phase") != "published":
            self.skipTest("current readiness record is not published")
        baseline = deepcopy(self.record)
        with patch(
            "tools.v017_draft_release_gates._git_show_text",
