...
class AssessmentWorkbookStarterTests(unittest.TestCase):
    ...
    def test_workbook_third_engagement_vignette_and_filled_trio_exist(self) -> None:
        vignette = self.workbook_root / "engagement3-vignette.example.md"
        example_root = self.workbook_root / "examples"
        filled = {
            "evidence-record": example_root
            / "engagement3-evidence-record.example.json",
            "assessment-result": example_root
            / "engagement3-assessment-result.example.json",
            "maturity-assessment": example_root
            / "engagement3-maturity-assessment.example.json",
        }
        text = vignette.read_text(encoding="utf-8")
        self.assertTrue(vignette.is_file())
        self.assertRegex(text, r"(?im)\bDraft\b")
        ...
        self.assertIn("EVD-ENG3-API100-GATEWAY", text)
        self.assertIn("ASR-ENG3-API100", text)
        self.assertIn("MAT-ENG3-API100", text)
        self.assertIn("#207", text)
        readme = (self.workbook_root / "README.md").read_text(encoding="utf-8")
        self.assertIn("engagement3-vignette.example.md", readme)
        ...
        for name, path in filled.items():
            with self.subTest(name=name):
                ...
                if name == "evidence-record":
                    ...
                    self.assertEqual(document.get("result_id"), "ASR-ENG3-API100")
                    self.assertEqual(document.get("status"), "draft")
                    self.assertIn(
                        "EVD-ENG3-API100-GATEWAY",
                        document.get("evidence_refs", []),
                    ...
                    self.assertEqual(document.get("maturity_id"), "MAT-ENG3-API100")
                    self.assertEqual(document.get("status"), "draft")
                    self.assertIn(
                        "ASR-ENG3-API100",
                        document.get("basis_refs", []),
...
class AssessmentEvidenceCatalogStarterTests(unittest.TestCase):
    ...
    def test_evidence_catalog_third_deepen_examples_exist(self) -> None:
        catalog = (self.catalog_root / "ESAF-1500-evidence-catalog.md").read_text(
            encoding="utf-8"
        )
        readme = (self.catalog_root / "README.md").read_text(encoding="utf-8")
        example_root = self.catalog_root / "examples"
        filled = {
            "record": example_root / "catalog3-record.example.json",
            "configuration": example_root / "catalog3-configuration.example.json",
            "observation": example_root / "catalog3-observation.example.json",
            "contract": example_root / "catalog3-contract.example.json",
            "external_assurance": example_root
            / "catalog3-external-assurance.example.json",
            "other": example_root / "catalog3-other.example.json",
        }
        self.assertIn("#207", readme)
        self.assertRegex(readme, r"(?im)third deepen|third Draft deepen")
        schema = json.loads(
            (SCHEMA_ROOT / "evidence-record.schema.json").read_text(encoding="utf-8")
        )
        validator = Draft202012Validator(
        ...
        for evidence_type, path in filled.items():
            with self.subTest(example=path.name):
                ...
                self.assertEqual(document.get("example_notice"), notice)
                self.assertTrue(
                    str(document.get("evidence_id", "")).startswith("EVD-CAT3-")
                )
                if evidence_type == "other":
                    self.assertTrue(
                        str(document.get("other_type_description", "")).strip()
                    )
...
class AssessmentAuditChecklistStarterTests(unittest.TestCase):
    ...
    def test_audit_checklist_third_sampling_vignette_exists(self) -> None:
        checklist = (
            self.checklist_root / "ESAF-1500-audit-checklist.md"
        ).read_text(encoding="utf-8")
        readme = (self.checklist_root / "README.md").read_text(encoding="utf-8")
        ...
        for path in filled_results:
            with self.subTest(example=path.name):
                ...
                self.assertEqual(document.get("status"), "draft")
                self.assertTrue(
                    str(document.get("result_id", "")).startswith("ASR-SAMP3-")
                )
