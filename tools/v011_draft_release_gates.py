#!/usr/bin/env python3
"""Validate the v0.11-draft publication-readiness record contract (Issue #129).
...
ISSUE = 129
RECORD_RELATIVE = (
    "docs/superpowers/reviews/2026-09-06-v011-draft-publication-readiness.md"
)
...
PREREQUISITE_SPECS = (
    ...
    (
        "audit_checklist",
        "draft_deepen",
        "audit_checklist_path",
        ("Draft deepen", "Draft"),
    ),
    (
        "governance_templates",
        "draft_deepen",
        "governance_templates_path",
        ("Draft deepen", "Draft deepen pack", "Draft"),
    ),
)
...
def derive_scope(root: Path) -> dict[str, object]:
    """Derive the v0.11-draft release scope from live, Git-tracked catalogs."""
...
def validate_record(root: Path, record: dict[str, object]) -> list[str]:
    """Return deterministic diagnostics for an in-memory v0.11-draft record."""
    errors: list[str] = []
...
def validate_readiness_body(body: str) -> list[str]:
    """Require the required v0.11-draft readiness body headings in order."""
    errors: list[str] = []
    cursor = 0
    positions: list[int] = []
