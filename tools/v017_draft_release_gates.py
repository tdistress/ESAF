#!/usr/bin/env python3
"""Validate the v0.17-draft publication-readiness record contract (Issue #208).
...
checks. Historical validators for earlier releases remain frozen.
"""
...
PREREQUISITE_SPECS = (
    ...
    (
        ...
        "working_draft",
        "esaf_1500_path",
        ("| 0.1.1 | Working Draft |", "Working Draft"),
    ),
    (
        "phase6_toolkit",
        "working_draft_third_deepen",
        "phase6_toolkit_path",
        ("third deepen", "Draft deepen"),
    ),
)
...
DRAFT_PROFILE_RELATIVE = "profiles/uk/0.2.0/profile.json"
...
ISO_IEC_42001_MATRIX_RELATIVE = (
    "docs/superpowers/specs/2026-09-07-iso-iec-42001-mapping-readiness-matrix.json"
)
NIST_CSF_MATRIX_RELATIVE = (
    "docs/superpowers/specs/2026-09-07-nist-csf-mapping-readiness-matrix.json"
)
ISO_IEC_27001_MATRIX_RELATIVE = (
    "docs/superpowers/specs/2026-09-11-iso-iec-27001-mapping-readiness-matrix.json"
)
NIST_SP_800_53_MATRIX_RELATIVE = (
    "docs/superpowers/specs/2026-09-16-nist-sp-800-53-mapping-readiness-matrix.json"
)
CIS_CONTROLS_MATRIX_RELATIVE = (
    "docs/superpowers/specs/2026-09-16-cis-controls-v8-mapping-readiness-matrix.json"
)
...
def derive_scope(root: Path) -> dict[str, object]:
    """Derive the v0.17-draft release scope from live, Git-tracked catalogs."""
    ...
    iso_matrix = _load_json(root / ISO_IEC_42001_MATRIX_RELATIVE)
    nist_csf_matrix = _load_json(root / NIST_CSF_MATRIX_RELATIVE)
    iso_27001_matrix = _load_json(root / ISO_IEC_27001_MATRIX_RELATIVE)
    nist_sp_800_53_matrix = _load_json(root / NIST_SP_800_53_MATRIX_RELATIVE)
    cis_controls_matrix = _load_json(root / CIS_CONTROLS_MATRIX_RELATIVE)
    return {
        ...
        "iso_iec_42001_disposition": iso_matrix.get("recorded_decision"),
        "nist_csf_disposition": nist_csf_matrix.get("recorded_decision"),
        "iso_iec_27001_disposition": iso_27001_matrix.get("recorded_decision"),
        "nist_sp_800_53_disposition": nist_sp_800_53_matrix.get("recorded_decision"),
        "cis_controls_disposition": cis_controls_matrix.get("recorded_decision"),
    }
...
def validate_record(root: Path, record: dict[str, object]) -> list[str]:
    """Return deterministic diagnostics for an in-memory v0.17-draft record."""
    errors: list[str] = []
...
def validate_readiness_body(body: str) -> list[str]:
    """Require the required v0.17-draft readiness body headings in order."""
    errors: list[str] = []
    cursor = 0
    positions: list[int] = []
