...
DEFAULT_OUTPUT = (
    ...
    / "superpowers"
    / "reviews"
    / "2026-09-07-nist-csf-2.0-mapping-go-no-go-review.md"
)
RIGHTS_REVIEW_PATH = (
    "docs/superpowers/reviews/2026-09-07-nist-csf-publication-rights-review.md"
)
...
QUESTION = (
    ...
    "one authorized, publishable NIST CSF 2.0 subcategory identifier, "
    "with each relationship's conditions, expected evidence, and known gaps "
    "recorded independently, without implying NIST CSF compliance, "
...
def _validate_reviewer_contract(value: object) -> None:
    ...
    if mapper != {
        ...
        "experience_required": ["NIST CSF 2.0", "ESAF-1600"],
    ...
    if (
        ...
        != "owner-approved NIST CSF subject-matter reviewer"
    ):
        raise ValueError("NIST CSF reviewer qualification is incomplete")
...
def validate_matrix(
    ...
    *,
    ...
    if (
        ...
        != "nist_csf_2_0_subcategory_identifier"
    ):
...
def render(matrix: dict[str, object]) -> str:
    ...
    lines = [
        ...
        "authorization to create a NIST CSF mapping.",
        "",
        ...
        "",
        "The official public NIST CSF 2.0 PDF digest, page count, and 106-subcategory "
        "inventory are recorded in the source oracle. PDF bytes are not committed to the "
        "repository.",
        "",
        "## Gate results",
        "",
    ...
    lines.extend(
        [
            ...
            "experience in NIST CSF 2.0 and ESAF-1600. The mapper may not review "
            "their own work.",
            "",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Render the mechanically derived NIST CSF readiness decision."
    )
