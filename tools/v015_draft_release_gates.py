#!/usr/bin/env python3
"""Validate the v0.15-draft publication-readiness record contract (Issue #185).
...
def derive_scope(root: Path) -> dict[str, object]:
    """Derive the v0.15-draft release scope from live, Git-tracked catalogs."""
...
def validate_record(root: Path, record: dict[str, object]) -> list[str]:
    """Return deterministic diagnostics for an in-memory v0.15-draft record."""
    errors: list[str] = []
...
def validate_readiness_body(body: str) -> list[str]:
    """Require the required v0.15-draft readiness body headings in order."""
    errors: list[str] = []
    cursor = 0
    positions: list[int] = []
