#!/usr/bin/env python3
"""Validate the v0.12-draft publication-readiness record contract (Issue #146).
...
PREREQUISITE_SPECS = (
    ...
    (
        ...
        "esaf_1000_path",
        ("| 0.2.1 | Working Draft |", "Working Draft"),
    ),
    (
        "esaf_1100",
        "working_draft_deepen",
        "esaf_1100_path",
        ("| 0.3.1 | Working Draft |", "Working Draft"),
    ),
)
...
def derive_scope(root: Path) -> dict[str, object]:
    """Derive the v0.12-draft release scope from live, Git-tracked catalogs."""
...
def validate_record(root: Path, record: dict[str, object]) -> list[str]:
    """Return deterministic diagnostics for an in-memory v0.12-draft record."""
    errors: list[str] = []
...
def validate_readiness_body(body: str) -> list[str]:
    """Require the required v0.12-draft readiness body headings in order."""
    errors: list[str] = []
    cursor = 0
    positions: list[int] = []
