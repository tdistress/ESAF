...
| Shared-serving and accelerator side-channel leakage across batch, cache, adapter, speculative-model, scheduler, HBM, P2P, DMA, RDMA, telemetry, profiler, crash-dump, backup, support, timing, occupancy, utilization, power and thermal paths | Demonstrate tenant and hardware isolation empirically across every shared execution, state, evidence and administrative plane | Repeated adversarial cross-tenant content and state tests for dynamic batch, cache, adapter routing, speculative models, scheduler, HBM and residual pages, P2P, DMA, RDMA, telemetry, profiler, crash dumps, backups and support paths, plus timing, occupancy, utilization, power and thermal side-channel measurements | Pass only when every named plane prevents cross-tenant content and state leakage and the exact hardware, firmware, driver, scheduler, device plugin, profiler and partition remain within approved empirical side-channel thresholds | Dedicate affected tiers or stop shared serving |
