...
The experience shall persistently identify the interaction as AI-assisted. Before processing begins it shall provide concrete, understandable notice of processing purposes, retention, possible human review, provider use, monitoring, and records status. It shall visibly disclose any material model, provider, grounding, memory, connector, or degraded-state change before, or when, that change affects the interaction; a silent change is prohibited.

The experience shall support keyboard operation, screen readers, logical focus order, semantic labels, zoom and reflow, color-independent status, localization, understandable reading level, manageable cognitive load and timeouts, and equivalent access to disclosures, context, citations, previews, confirmations, errors, and feedback. It shall not claim human identity, emotion, consciousness, authority, or relationship, or use fabricated certainty, hidden advertising, manufactured urgency, emotional manipulation, dark patterns, or misleading controls to induce disclosure, reliance, confirmation, or continued use.
...
Attachments, pasted text, URLs, images, archives, code, office documents, imported conversations, and connector responses are isolated and treated as untrusted. Actual and declared type, size, nesting, compression, active content, links, OCR, objects, metadata, classification, ownership, provenance, inspection, and purpose are validated. Unsupported, encrypted, malformed, polyglot, or high-risk content shall be rejected or sent to an approved protected process. Least-privilege parsers and extractors use bounded resources and restricted networking, never execute active content, and cannot implicitly follow embedded links, macros, instructions, credentials, or external references. URL handling resists SSRF, redirects, unsafe downloads, credentials, tracking, and egress.
...
|---|---|
| Experience shell | Approved channels, persistent AI identification and material-change notices, accessible and non-deceptive interaction, context controls, content-origin labels, citations, previews, confirmations, status, challenge, feedback, export, and session controls |
...
| CP3 | User notice and AI disclosure | Persistent AI identification, concrete processing/retention/review/provider/monitoring/records notice, and visible material-change disclosure keep users informed | Compliance; Product, UX, Legal, and Accessibility |
...
| CP5 | Attachment and imported-content inspection | Untrusted content is validated and isolated; unsafe forms are rejected or protected; parsers cannot execute or implicitly follow content | Application Owner; Security Engineering |
...
Assessment inspects machine-readable and user-visible context manifests, mandatory-context protection, context-change invalidation, hidden-instruction inventory and integrity, attachment and URL isolation and disposition, parser non-execution and non-following behavior, prompt injection, discovery/retrieval/presentation/sharing/export authorization, recipient access, inaccessible-source inference, citation correctness and authorization-safe presentation, and output controls at display, copy, download, share, email, export, and connector boundaries.

Evidence covers memory opt-in and item provenance, correction/deletion/hold/isolation, action-manifest integrity, protected-value inspection, accessible semantic preview, step-up/dual approval, exact binding, idempotency and unknown-outcome reconciliation, persistent AI identification and material-change notices, concrete processing/retention/review/provider/monitoring/records notices, keyboard/screen-reader/focus/label/zoom/reflow/color/localization/reading-level/cognitive-load/timeout equivalence, deceptive-UX tests, feedback anti-retaliation, minimized decision records, records/privacy/monitoring controls, and provider/connector gap registers.
