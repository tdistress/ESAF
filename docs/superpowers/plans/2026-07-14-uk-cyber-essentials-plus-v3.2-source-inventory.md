...
- exact equality with `RIGHTS_ELEMENTS`; permitted/prohibited disjointness and exhaustiveness; `permitted_elements == list(RIGHTS_ELEMENTS)`; `prohibited_elements == []`; `copied_requirement_or_passage_text_prohibited is True`; and `allowed_verbatim_locations == ["known_anomalies[0].source_literal"]`;
...
- exactly one anomaly whose `source_literal == "tests 2 to 7"`; a recursive path-and-value walk finding that literal exactly once at the whitelisted path and nowhere else; and no provision summary or other free-text field matching copied requirement text or source passages from the pinned canonical PDF;
